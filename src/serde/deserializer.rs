...
impl<'de> Deserializer<'de> {
    ...
    pub const fn from_bytes(input: Iter<'de, u8>) -> Self {
        Deserializer { input }
    }

    pub fn next_byte(&mut self) -> Result<u8> {
        self.input.next().ok_or(Error::Eof).map(|byte| *byte)
    }
}
...
where
    T: Deserialize<'a>,
{
    let mut deserializer = Deserializer::from_bytes(s.iter());
    let t = T::deserialize(&mut deserializer)?;
    if deserializer.input.peekable().peek().is_some() {
        Err(Error::ResidualBytes)
    } else {
        Ok(t)
    }
}
...
impl<'de, 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_bool(self.next_byte().map(|byte| byte != 0)?)
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_i8(i8::from_le_bytes([self.next_byte()?]))
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_i16(i16::from_le_bytes([self.next_byte()?, self.next_byte()?]))
    }
    ...
    where
        V: Visitor<'de>,
    {
        visitor.visit_u16(u16::from_le_bytes([self.next_byte()?, self.next_byte()?]))
    }
